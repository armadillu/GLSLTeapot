...
void main( void ){
	...
	float eyeIn = shaderVal1 * pow( dot(L,E) * dot(E,N), shaderVal2 * 60.0);
	vec4 H = gl_LightSource[0].specular * gl_FrontMaterial.specular * clamp(eyeIn, 0.0, 1.0);	

	float n = shaderVal3 * snoise(E * N * shaderVal2);
	float n2 = shaderVal3 * snoise( N * shaderVal1) + n;
	vec4 F = vec4( -n2, n2, -n2, 1.0);
	//final_color += T;
	final_color += F;
